...
{
    return (const char *)"Red Pitaya ohs law app application.\n";
}

int rp_app_init(void)
{
    fprintf(stderr, "Loading test app version %s-%s.\n", VERSION_STR, REVISION_STR);
...
{
    fprintf(stderr, "Unloading oh laws app version %s-%s.\n", VERSION_STR, REVISION_STR);
...
int rp_copy_params(rp_app_params_t *src, rp_app_params_t **dst)
{
    ...
    //Change shunt on client request
    if (rp_main_params[CHANGE_SHUNT].value == 1){
      i2c_set_shunt(rp_main_params[SHUNT_VAL].value);
      rp_main_params[CHANGE_SHUNT].value = 0;
    }

    //Change led on client request
    if(rp_main_params[CHANGE_LED].value == 1){
      set_led(rp_main_params[SET_LED].value);
      rp_main_params[CHANGE_LED].value = 2;
    }
